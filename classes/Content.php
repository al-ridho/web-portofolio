...
class Content extends DBConnection
{
	...
	public function trends()
	{
		...
		if (empty($id)) {
			$sql = "INSERT INTO trends set $data";
		} else {
			$sql = "UPDATE trends set $data where id = {$id}";
		}
		$save = $this->conn->query($sql);
		$action = empty($id) ? "added" : "updated";
		if ($save) {
			$resp['status'] = 'success';
			$resp['message'] = " Trends Details successfully " . $action;
			$this->settings->set_flashdata('success', $resp['message']);
		} else {
			$resp['status'] = 'failed';
			$resp['message'] = " error:" . $sql;
	...
	{
		extract($_POST);
		$qry = $this->conn->query("DELETE FROM trends where id = $id");
		if ($qry) {
			$resp['status'] = 'success';
			$resp['message'] = " Trends Details successfully deleted";
			$this->settings->set_flashdata('success', $resp['message']);
		}
	}
